<?php
	...
	if (!empty($_POST["record-id"])) {
		...
		// sql päring
		$sql = 'UPDATE channel_ideas SET ' . $_POST["col"] . ' = ' . $_POST["new"] . ' WHERE ID=' . $_POST["record-id"];
...
?>
<form method="post" action="index.php?ideas=1" name="form" id="form1" enctype="multipart/mixed">
...
<div class="form-floating">
	<select class="form-select my-3" id="col" name="col"/>
	<?php
		include("../../connect.php");
		$query = "SELECT * FROM channel_ideas";
		$result = mysqli_query($connection, $query);
...
<div class="form-floating">
<textarea class="form-control my-3" id="text1" name="new" rows="5" cols="100"><?php
if ((!empty($_GET["mod"])) && (!empty($_GET["id"]))) {
	include("../connect.php");
	$query = "SELECT " . $_GET["mod"] . " FROM channel_ideas WHERE ID = " . $_GET["id"];
	$result = mysqli_query($connection, $query);
