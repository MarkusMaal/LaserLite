...
function display_property($row, $item, $idx) {
	if ((is_numeric($row[$item])) && ($item != 0) && ($item != 5)) {
...
if (empty($_GET["nopages"])) {
	$disp_cols = array(1, 2, 3, 4, 5, 6);
} else {
	if ($_GET["nopages"] == "1") {
		$disp_cols = array(0, 1, 2, 3, 4, 5, 6);
	}
}
$search = "";
if (!empty($_GET["q"])) {
...
$query = $query . ' CONCAT(ID, Video, Kirjeldus, Klass) LIKE "%' . $search . '%"';
...
$query = str_replace('CONCAT(ID, Video, Kirjeldus, Klass) LIKE "%%" AND', '', $query);
$query = str_replace('CONCAT(ID, Video, Kirjeldus, Klass) LIKE "%%"', '', $query);
...
if(($_SERVER["REQUEST_URI"] != "/channel_db/ideas/index.php") && ($_SERVER["REQUEST_URI"] != "/channel_db/ideas/")) {
	...
	if ($ignorepages == false) {
		...
		if ($channel == "all") {
			$channels = mysqli_query($connection, "SELECT DISTINCT Kanal FROM channel_ideas");
		...
		$deletes = mysqli_query($connection, "SELECT DISTINCT Valmis FROM channel_ideas");
		if ($done == 0) {
			echo '<td class="filterbar"><h2>' . ms("Done", "Valmis") . '?</h2><ul>';
			...
			if ($done == 1) {
				echo '<td class="filterbar"><h2>' . ms("Done", "Valmis") . '?</h2><p>'. y() . '</p></td>';
			} else {
				echo '<td class="filterbar"><h2>' . ms("Done", "Valmis") . '?</h2><p>' . n() . '</p></td>';
			}
		}
		
		$deletes = mysqli_query($connection, "SELECT DISTINCT Ülekanne FROM channel_ideas");
		if ($live == 0) {
