...
function inv_remove() {
    if (game.remove) {
        game.remove = false;
        document.getElementById("btype").innerHTML = "Mode: Draw";
    } else {
        game.remove = true;
        document.getElementById("btype").innerHTML = "Mode: Eraser";
    }
}
...
function iterate() {
    if (game.pause) {
        ...
        document.getElementById("state").innerHTML = "Simulation is running...";
        document.getElementById("pause_resume").innerHTML = "Pause simulation";
        clearInterval(game.fps);
		game.fps = setInterval(game.update, game.framerate);
    } else {
        game.pause = true;
        game.framerate = 16;
        document.getElementById("pause_resume").innerHTML = "Continue simulation";
        clearInterval(game.fps);
		game.fps = setInterval(game.update, game.framerate);
    }
}
...
function reload_game() {
    clearInterval(game.fps);
    document.getElementById("btn_parent").innerHTML = '<a href="#/" onclick="iterate()" id="pause_resume" class="listitems">Start simulation</a><a href="#/" onclick="iterate_one()" id="pausnupp" class="listitems" style="margin-left: 5px;">Next iteration</a><a href="#/" onclick="reload_game()" class="listitems" style="margin-left: 5px;">New game</a><a style="margin-left: 5px;" href="#/" onclick="inv_remove()" class="listitems" id="btype">Mode: Draw</a>';
    document.getElementById("it_str").innerHTML = 'Press "Start simulation" to begin';
    document.getElementById("state").innerHTML = 'Drawing mode';
    document.getElementById("sizesetup").innerHTML = '<span style="margin-right: 10px;">Rectangle size:</span><input style="padding: 10px; border: none; font-size: 12pt; width: 25px;" value="5" id="res_w"></input><a href="#/" style="margin-left: 10px;" onclick="game.set_size()" class="listitems">Apply</a>';
    game.lc_down = false;
    game.pause = true;
    game.frame = 0;
    game.block_x = 1;
