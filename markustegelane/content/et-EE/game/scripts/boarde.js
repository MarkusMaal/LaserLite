function Board(w, h, tlsize) {
    ...
    this.updategrid = function () {
        ...
        if (!game.pause) {
            ...
            if (!game.over)
            {
                document.getElementById("it_str").innerHTML = "Time: " + String(game.frame) + " time unit(s)";
            }
            if (game.autopause) {
                game.pause = true;
            ...
            if (this.get_count(newboard) == 0) {
                document.getElementById("state").innerHTML = "Game over: Entropy";
                document.getElementById("btn_parent").innerHTML=' <a href="#/" onclick="reload_game()" class="listitems">Try again</a>';
                game.pause = true;
                game.over = true;
            } else {
                if (this.compare_lists(newboard, this.boardgrid)) {
                    document.getElementById("state").innerHTML = "Game over: No moving cells";
                    document.getElementById("btn_parent").innerHTML=' <a href="#/" onclick="reload_game()" class="listitems">Try again</a>';
                    game.pause = true;
                    game.over = true;
                }
                    ...
                    if ((this.compare_lists(game.last_two, newboard)) || (this.compare_lists(game.last_four, newboard))) {
                        document.getElementById("state").innerHTML = "Game over: Frozen evolution";
                        document.getElementById("btn_parent").innerHTML=' <a href="#/" onclick="reload_game()" class="listitems">Try again</a>';
                        game.over = true;
                    }
                }
