        ...
        if (obstacles.length > 0) {
            for(var pc = 0; pc < obstacles.length; pc++) {
                ...
                if(collide(plr.x, plr.y, plr.w, plr.h, obstacles[pc].x, obstacles[pc].y, obstacles[pc].w, obstacles[pc].h) == true) {
                    ...
                    obstacles[pc].x = game.cnv.width + obstacles[pc].w;
                    plr.y = 270;
                    document.getElementById("hs").innerHTML = "Game over";
                    if (continues > 0) {
                        document.getElementById("pausnupp").innerHTML = "Continue (" + String(continues) + ")";
                        continues -= 1;
                    }
                    else {
                    ...
                    document.getElementById("hs").innerHTML = "To jump press Return or tap the game area<br/>Press Esc to pause/continue<br/>Press F5 to start a new game";
                }
                obstacles[pc].update();
            }
        }
        document.getElementById("pts").innerHTML = "Points: " + String(position);
        plr.update();
    }
}
var sd = 10;
...
function pausegame() {
    if (paused == false) {
        if (document.getElementById("pausnupp").innerHTML == "Continue (" + String(continues) + ")") {
            game.interval = setInterval(game.update, 16);
            document.getElementById("pausnupp").innerHTML = "Pause";
            document.getElementById("hs").innerHTML = "Game is currently paused. Press continue to continue the game.";
        } else {
            paused = true;
            document.getElementById("pausnupp").innerHTML = "Continue";
            clearInterval(game.interval);
        }
    } else {
        paused = false;
