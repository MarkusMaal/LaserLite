...
let game = {
	...
	over : () => {
		document.querySelector("#next").innerHTML = "Game over"
		document.querySelector("#pauseButton").style.display = "none"
	...
	loop : () => {
		if (!this.paused) {
			...
			floating_brick.fall()
			game.screen()
			document.querySelector("#score").innerHTML = `Score: ${this.score}<br/>High score: ${this.highscore}`
		}
	},
...
let floating_brick = {
	...
	fall: () => {
		...
		document.querySelector("#debug_speed").innerHTML = "Level: " + String(11 - game.getSpeed())
		//document.querySelector("#debug_pos").innerHTML = "Position: " + this.position
		//document.querySelector("#debug_fpos").innerHTML = "Visual position: <br/>" + this.fakeposition
	},
	randomize: () => {
		this.position[1] = 0
